...
class PDF extends FPDF {
    function Header() {
        ...
        $width = $this->w - 2 * $this->lMargin;  // Ancho de la página menos los márgenes izquierdo y derecho
        $this->Rect($this->lMargin, 0, $width, 20, 'F');  // Usar el ancho calculado
    ...
    function SectionContent($label, $content) {
        ...
        $this->MultiCell(0, 6, utf8_decode($content));
        $this->Ln(2);
    }
    ...
    function DrawTableHeader($headers) {
        ...
        foreach ($headers as $header) {
            $this->Cell(40, 8, utf8_decode($header), 1, 0, 'C', true);
        }
        $this->Ln();
    }
    ...
    function DrawTableRow($data) {
        ...
        foreach ($data as $item) {
            $this->Cell(40, 6, utf8_decode($item), 1, 0, 'C');
        }
        $this->Ln();
    }

    function DrawLine() {
        $this->SetDrawColor(169, 169, 169); // Gris
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST)) {
    ...
    $pdf->SectionContent("Nombre", $_POST['rfcReceptor'] ?? '');
    $pdf->SectionContent("RFC", $_POST['razonSocialReceptor'] ?? '');
    $pdf->SectionContent("Domicilio", $_POST['domicilioReceptor'] ?? '');
    $pdf->DrawLine();

    // Datos del Comprobante
    $pdf->SectionTitle("Datos del Comprobante");
    $pdf->SectionContent("Método de Pago", $_POST['metodoPago'] ?? '');
    $pdf->SectionContent("Uso de CFDI", $_POST['usoCfdi'] ?? '');
    $pdf->SectionContent("Moneda", $_POST['moneda'] ?? '');
    $pdf->DrawLine();
